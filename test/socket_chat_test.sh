...
echo "Testing socket chat (TCP)"

# Uruchom serwer w tle
echo "Starting server..."
./socket_chat_server 127.0.0.1 12345 > server.log 2>&1 &
SERVER_PID=$!
sleep 1
...
(
  ...
  sleep 1
  echo "STOP"
) | ./socket_chat_client Anna 127.0.0.1 12345 > anna.log 2>&1 &
...
(
  ...
  sleep 2
  echo "STOP"
) | ./socket_chat_client Bob 127.0.0.1 12345 > bob.log 2>&1 &
...
echo "=== END OF LOGS ==="
echo
echo "Socket chat test (TCP) finished"
echo "----------------------------------------"